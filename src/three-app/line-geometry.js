...
export class LineGeometry extends BufferGeometry {
  ...
  _createBufferAttributes(numPoints) {
    ...
    this.setAttribute(
      "position",
      new Float32BufferAttribute(attrPositionLength, attrPositionItemSize)
    );
    ...
    this.setIndex(
      new Uint16BufferAttribute(attrIndexLength, attrIndexItemSize)
    );
  }
  ...
  update(path, closed) {
    ...
    if (!closed && this._extendEnds && path.length >= 2) {
      ...
      const pExtendedStart = v1
        .clone()
        .addScaledVector(vExtendStartDirection, this._lineThickness)
        .toArray();
      ...
      const pExtendedEnd = v4
        .clone()
        .addScaledVector(vExtendEndDirection, this._lineThickness)
        .toArray();
    ...
    path.forEach((point, pointIndex) => {
      ...
      p1.add(
        new Vector2(nx, ny).multiplyScalar(
          this._halfLineThickness * -mitreClamped
        )
      );
      p2.add(
        new Vector2(nx, ny).multiplyScalar(
