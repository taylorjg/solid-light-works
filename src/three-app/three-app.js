...
const LEAVE_TIMELINE_SCRUBBING_MODE_EVENT_NAME =
  "leave-timeline-scrubbing-mode";
...
const emitEnterTimelineScrubbingMode = (
...
const reportCameraPosition = () => {
  ...
  const newVector3 = (vec3) =>
    `new THREE.Vector3(${prec2(vec3.x)}, ${prec2(vec3.y)}, ${prec2(vec3.z)})`;
  console.log(
    `{ position: ${newVector3(camera.position)}, target: ${newVector3(controls.target)} }`
  );
};
...
const updateVisibility = () => {
  installations.forEach((installation, index) => {
    ...
    installation.updateVisibility(mode, isCurrentInstallation, {
      vertexNormalsEnabled,
      intersectionPointsEnabled,
      formBoundariesEnabled,
    });
...
const switchCameraPose = (reset) => {
  ...
  const cameraPoses = allCameraPoses.filter((cameraPose) =>
    behindOnly ? cameraPose.isBehind : true
  );
  if (reset) {
...
const toggleTimelineScrubbingMode = () => {
...
const setTimelineScrubbingMode = (value) => {
  ...
  inTimelineScrubbingMode
    ? enterTimelineScrubbingMode()
    : leaveTimelineScrubbingMode();
  emitSettingsChanged();
};
...
const setTimelineScrubbingValue = (timelineScrubbingValue) => {
  const currentInstallation = installations[currentInstallationIndex];
  currentInstallation.updateRenderables(
    mode,
    undefined,
    timelineScrubbingValue
  );
...
export const threeAppInit = async () => {
  ...
  const hazeTexture = await U.loadTexture(
    `${import.meta.env.BASE_URL}haze.jpg`
  );
  ...
  installations = configs.map(
    (config) => new Installation(scene, resources, config)
  );
  ...
  const onDocumentKeyDownHandler = (e) => {
    ...
    switch (e.key) {
      case "a":
        return toggleAxes();
      case "b":
        return toggleBehindOnly();
      case "c":
        return reportCameraPosition();
      case "d":
        return showRendererInfo();
      case "e":
        return toggleFormBoundaries();
      case "f":
        return switchInstallation();
      case "i":
        return toggleIntersectionPoints();
      case "m":
        return toggleMode();
      case "n":
        return toggleShowName();
      case "p":
        return switchCameraPose();
      case "r":
        return toggleAutoRotate();
      case "s":
        return toggleStats();
      case "t":
        return toggleTimelineScrubbingMode();
      case "v":
        return toggleVertexNormals();
      default:
        return;
    }
  };
