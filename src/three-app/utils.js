...
export const sumBy = (xs, selector) => sum(xs.map(selector));
...
export const isClose = (a, b, tolerance = 0.01) => Math.abs(a - b) <= tolerance;
...
export const isCloseVec = (a, b, tolerance) => {
  ...
  return range(length).every((index) =>
    isClose(componentsA[index], componentsB[index], tolerance)
  );
};
