...
const makeWork1 = () => {
  const transformForm1 = new THREE.Matrix4()
    ...
    .premultiply(
      new THREE.Matrix4().makeTranslation(-4.8, 1.3 + FRAME_THICKNESS, 3.25)
    );

  const transformForm2 = new THREE.Matrix4()
    .premultiply(new THREE.Matrix4().makeRotationY(-C.HALF_PI))
    .premultiply(
      new THREE.Matrix4().makeTranslation(0.8, 1.3 + FRAME_THICKNESS, 6.95)
    );
    ...
    formConfigs: [
      {
        ...
        config2D: {
          transform: new THREE.Matrix4().multiply(
            new THREE.Matrix4().makeTranslation(-2.5, 0, 0)
          ),
        },
        config3D: {
...
export const config = {
  ...
  config3D: {
    ...
    cameraPoses: [
      {
        position: new THREE.Vector3(-2.82, 2.26, 13.96),
        target: new THREE.Vector3(-2.25, 0.78, 5.36),
      },
      {
        position: new THREE.Vector3(-9.72, 2.13, 8.7),
        target: new THREE.Vector3(-2.91, 1.21, 5.71),
      },
    ],
    scenery: [
      new Floor(22.4, 12.6),
