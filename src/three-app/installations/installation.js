...
export class Installation {
  ...
  updateRenderables(mode, deltaMs, absoluteMs) {
    for (const work of this.config.works) {
  ...
  _createRenderables3D(resources) {
    ...
    this.config.config3D.scenery.forEach((sceneryItem) =>
      sceneryItem.create(this.sceneryGroup)
    );
