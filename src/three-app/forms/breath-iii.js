...
const MAX_NUM_POINTS =
  3 * TRAVELLING_WAVE_POINT_COUNT + 2 * ELLIPSE_POINT_COUNT + 1 + 2;
...
export class BreathIIIForm {
  ...
  getFootprintData() {
    ...
    const rx =
      ...
      0.25 *
        Math.sin(
          (C.PI / BREATH_CYCLE_TICKS) * (this.tick % BREATH_CYCLE_TICKS)
        );
    ...
    const cwCurve = (angle1, angle2) => {
      ...
      return normalisedAngle1 > normalisedAngle2
        ? getEllipseSegmentPoints(
            negateAngle(normalisedAngle1),
            normalisedAngle2
          )
        : getEllipseSegmentPoints(
            normalisedAngle1,
            negateAngle(normalisedAngle2)
          );
    };
    ...
    if (intersections.length === 2 || intersections.length === 3) {
      ...
      if (intersections.length === 3) {
        ...
        for (const intersection of intersections) {
          ...
          if (
            normalisedAngle > C.HALF_PI &&
            normalisedAngle <= C.HALF_PI + C.PI
          ) {
      ...
      const travellingWavePoints1 = getTravellingWaveSegmentPoints(
        0,
        intersection1.t2
      );
      const travellingWavePoints2 = getTravellingWaveSegmentPoints(
        intersection2.t2,
        this.width
      );

      const linePoints = U.combinePoints(
        travellingWavePoints1,
        ellipsePoints,
        travellingWavePoints2
      );
    ...
    if (intersections.length === 4) {
      const [intersection1, intersection2, intersection3, intersection4] =
        intersections;

      const travellingWavePointsStart = getTravellingWaveSegmentPoints(
        0,
        intersection1.t2
      );
      const travellingWavePointsMiddle = getTravellingWaveSegmentPoints(
        intersection2.t2,
        intersection3.t2
      );
      const travellingWavePointsEnd = getTravellingWaveSegmentPoints(
        intersection4.t2,
        this.width
      );
      ...
      if (isTroughBelowEllipse()) {
        ...
        const ellipsePointsBottom = ccwCurve(
          intersection2.t1,
          intersection3.t1
        );
        ...
        const linePoints2 = U.combinePoints(
          travellingWavePointsMiddle,
          ellipsePointsBottom
        );
        const line2 = new Line(linePoints2, {
          ...commonLineOptions,
          closed: true,
        });
        ...
        const ellipsePointsLeft = smallCurve(
          intersection1.t1,
          intersection2.t1
        );
        const ellipsePointsRight = smallCurve(
          intersection3.t1,
          intersection4.t1
        );
    ...
    const line2 = new Line(travellingWavePoints, {
      ...commonLineOptions,
      clipToFormBoundary: true,
    });
    const lines = [line1, line2];
