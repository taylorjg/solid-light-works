...
export class SkirtIIIForm {
  ...
  getFootprintData() {
    ...
    const xoffsets = [xoffsetPrimary, xoffsetSecondary];
    ...
    const findEyeWaveIntersection = (
      primaryOrSecondary,
      topOrBottom,
      angle
    ) => {
      const parametricEyeWaveXFn = parametricEyeWaveXFns[primaryOrSecondary];
      const parametricEyeWaveXDerivativeFn =
        parametricEyeWaveXDerivativeFns[primaryOrSecondary];
      const parametricEyeWaveYFn = parametricEyeWaveYFns[topOrBottom];
      const parametricEyeWaveYDerivativeFn =
        parametricEyeWaveYDerivativeFns[topOrBottom];
    ...
    const tryVariousAngles = (primaryOrSecondary, topOrBottom, ...angles) => {
      for (const angle of angles) {
        const intersection = findEyeWaveIntersection(
          primaryOrSecondary,
          topOrBottom,
          angle
        );
    ...
    const intersectionPoints = intersections.map(
      (intersection) =>
        new THREE.Vector2(
    ...
    if (intersections.length === 4) {
      const eyeWavePointsPrimary = getEyeWavePointsPrimary(
        intersection1.t2,
        intersection2.t2
      );
      const eyeWavePointsSecondary = getEyeWavePointsSecondary(
        intersection3.t2,
        intersection4.t2
      );
      const ellipsePointsTop = getEllipsePoints(
        (C.PI * 5) / 8,
        intersection1.t1
      );
      const ellipsePointsBottom = getEllipsePoints(
        intersection4.t1,
        C.PI + (C.PI * 5) / 8
      );

      const topPoints = U.combinePoints(ellipsePointsTop, eyeWavePointsPrimary);
      const bottomPoints = U.combinePoints(
        eyeWavePointsSecondary,
        ellipsePointsBottom
      );
